...
export const updateProfile = async (req, res) => {
  try {
    ...
    if (skills) {
      const skillArray = skills.split(",").map(skill => skill.trim()); // Trimming spaces around skills
      user.profile.skills = skillArray;
    }

    if (file) {
      // Ensure that the file is in the correct format (PDF)
      ...
      try {
        const fileUri = getDataUri(file); // Assuming getDataUri handles file transformation
        const cloudResponse = await cloudinary.uploader.upload(fileUri.content);
    ...
    console.error(error.stack); // For debugging purposes
    return res.status(500).json({
